...
def crop_resize_v2(image: torch.Tensor,
    ...
    Args:
        ...
        out_size: int
        fill: Important. 
            If cropping a GT mask, and if we set -1 to ignore, 
            fill should be -1 since we want GT doesn't have out-of-boundary pixels. and we want to ignore them.
    ...
    transform = transforms.Compose(
        [transforms.Pad([pad_x, pad_y], fill=fill)])
    x += pad_x
    y += pad_y
...
def batch_crop_resize(images: Union[np.ndarray, torch.Tensor],
                      boxes: torch.Tensor,
                      out_size: int,
                      fill=0) -> Union[np.ndarray, torch.Tensor]:
    ...
    for i in range(bsize):
        ...
        # set out-of-boundary pixel to -1
        img_out = crop_resize_v2(img, boxes[i], out_size, fill=fill)  
        if is_np:
